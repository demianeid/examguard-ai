...
// بيانات افتراضية للامتحان الحالي (يمكن استبدالها ببيانات فعلية من API)
const initialExamData: ExamFormData = {
  examTitle: "Midterm Exam - Data Structure",
  selectedClass: "class-10",
  description: "This exam covers fundamental data structure concepts including arrays, linked lists, and trees.",
  duration: "90",
  totalMarks: "100",
  ...
  endDate: "2024-12-15",
  endTime: "10:30",
  instructions: "1. No calculators allowed\n2. Answer all questions\n3. Time limit: 90 minutes",
};
...
const initialQuestions: Question[] = [
  ...
  },
  {
    id: "2",
    type: "true-false",
    text: "Binary trees can have more than two children per node.",
    options: ["True", "False"],
    marks: "3",
    correctAnswer: 1,
  },
...
const initialSecuritySettings: SecurityFeature[] = [
  {
    id: "1",
    name: "AI Proctoring",
...
export default function EditExam() {
  ...
  const [formData, setFormData] = useState<ExamFormData>(initialExamData);
  const [questions, setQuestions] = useState<Question[]>(initialQuestions);
  const [errors, setErrors] = useState<Partial<Record<keyof ExamFormData, string>>>({});
  const [securitySettings, setSecuritySettings] = useState<SecurityFeature[]>(initialSecuritySettings);
  ...
  // تتبع التغييرات
  useEffect(() => {
    const hasFormChanges = JSON.stringify(formData) !== JSON.stringify(initialExamData);
    const hasQuestionChanges = JSON.stringify(questions) !== JSON.stringify(initialQuestions);
    const hasSecurityChanges = JSON.stringify(securitySettings) !== JSON.stringify(initialSecuritySettings);
    
    setHasChanges(hasFormChanges || hasQuestionChanges || hasSecurityChanges);
  }, [formData, questions, securitySettings]);
    ...
    for (const question of questions) {
      ...
      if (question.type === "multiple-choice") {
        ...
        if (question.correctAnswer === undefined) {
          alert(`Please select correct answer for question: "${question.text.substring(0, 50)}..."`);
          return false;
        }
      }
  ...
  const handleClose = () => {
    if (hasChanges) {
      const confirmClose = window.confirm(
        "You have unsaved changes. Are you sure you want to close? All unsaved changes will be lost."
      );
      if (!confirmClose) {
        return;
      }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-lg max-w-3xl w-full p-8">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Edit and update your exam configuration
            </p>
            {hasChanges && (
            ...
            <div className="bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-lg p-6 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-blue-100 mt-1">
                  Update proctoring features based on your exam requirements
                </p>
              </div>
            </div>
        ...
        <div className="flex justify-between items-center mt-8 pt-6 border-t border-gray-200">
          <div className="flex gap-2">
            ...
            {hasChanges && (
              <button
                ...
                className="px-4 py-2 rounded-md font-medium text-gray-700 hover:bg-gray-200 transition-colors border border-gray-300"
          ...
          <div className="flex gap-2">
            {hasChanges && currentStep === 4 && (
              <button
                onClick={handleSaveChanges}
                disabled={isLoading}
                className="px-6 py-2 rounded-md font-medium text-white bg-green-600 hover:bg-green-700 transition-colors flex items-center gap-2 disabled:opacity-50"
            ...
            <button
              ...
              aria-label={currentStep === 4 ? "Save exam changes" : "Go to next step"}
            >
