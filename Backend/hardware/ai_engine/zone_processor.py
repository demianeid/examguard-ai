...
# ── Zone-inference thread pool ─────────────────────────────────────────────────
# One worker per zone — lets all student zones within a camera frame be
# inferred in parallel. YOLO and MediaPipe instances are singletons shared
# across threads; Python's GIL is released during native C/C++ inference so
# real concurrency is achieved for CPU-bound detection code.
_ZONE_THREAD_POOL = ThreadPoolExecutor(max_workers=32, thread_name_prefix="zone_infer")
_zone_pool_lock   = threading.Lock()  # guards singleton creation in _ensure_detectors_loaded
...
def _infer_zone(zc: ZoneCrop) -> ZoneCrop:
    """
    Run all three detectors on a single ZoneCrop.
    Executed inside the zone thread pool — safe to call concurrently.
    """
    det_kwargs = dict(
        scale_x      = zc.scale_x,
        scale_y      = zc.scale_y,
    ...
    try:
        zc.alerts += _head_pose.estimate(zc.model_input, zone_id=zc.zone_id)
    except Exception as exc:          # noqa: BLE001
...
def process_frame(
    ...
    """
    Top-level entry point called by ``runpod_worker.handler`` or local dispatcher.
    All valid zones are inferred IN PARALLEL so that processing time is
    roughly constant regardless of how many students are in the frame.
    """
    ...
    # 2. Crop all zones (fast — just NumPy slicing, no model inference yet)
    zone_crops = crop_zones(frame, zones, target_size)

    # 3. Ensure detector singletons are loaded before spawning threads
    _ensure_detectors_loaded()

    # 4. Submit each valid zone to the thread pool for parallel inference
    valid_crops   = [zc for zc in zone_crops if zc.is_valid]
    invalid_crops = [zc for zc in zone_crops if not zc.is_valid]

    if valid_crops:
        futures = {_ZONE_THREAD_POOL.submit(_infer_zone, zc): zc for zc in valid_crops}
        inferred: dict[int, ZoneCrop] = {}
        for future in as_completed(futures):
            original_zc = futures[future]
            try:
                inferred[original_zc.zone_id] = future.result()
            except Exception as exc:
                logger.error("Zone %s inference thread crashed: %s", original_zc.zone_id, exc)
                inferred[original_zc.zone_id] = original_zc  # return unmodified on crash

        # Preserve original zone order
        result_crops = [
            inferred.get(zc.zone_id, zc) if zc.is_valid else zc
            for zc in zone_crops
        ]
    else:
        result_crops = zone_crops

    logger.info(
        "process_frame() complete | %d valid zones inferred in parallel.",
        len(valid_crops),
    )

    # 5. Serialise
    return [zc.to_dict() for zc in result_crops]
