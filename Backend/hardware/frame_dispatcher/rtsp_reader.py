...
class RtspReader:
    ...
    def _capture_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            # Constantly grab frames to empty OpenCV's internal buffer
            # This ensures we always get real-time frames with zero delay.
            ret = self._cap.grab()
            if not ret:
            ...
            # Only decode the frame (which takes CPU) if we hit our target FPS interval
            if elapsed >= interval:
